...
int CmdGetSysI(char *Inbuf)
{
    ...
    sprintf_s(Inbuf, S3_MAX_GPIB_RET_LEN,
        ...
        S3Data->m_SleepAllPending,
		S3Data->m_WakeAllPending,
		S3Data->m_Locked);			// 36
    return 0;
}
...
int CmdGetTXMod(char *Inbuf, int Rx, int Tx)
{
		...
		(Tx != -1 && S3TxValidQ(Rx, Tx) && S3Data->m_Rx[Rx].m_Tx[Tx].m_Type != S3_TxUnconnected))
    {
        sprintf_s(Inbuf, S3_MAX_GPIB_RET_LEN,
			...
			S3Data->m_Rx[Rx].m_Tx[Tx].m_RLLStableCnt,				// 35
            ...
            S3Data->m_Rx[Rx].m_Tx[Tx].m_Config.m_WindowTracking,	// 41
            ...
            S3Data->m_Rx[Rx].m_Tx[Tx].m_EmergencySleep,				// 48
