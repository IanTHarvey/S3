...
#define S3_IMG_UPDATE_DEST			"Y:\\Project files\\Sentinel3\\Software\\LatestUpdates\\"
...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	wchar_t	ImgUpdateDest[MAX_PATH];

	swprintf_s(ImgUpdateDest, MAX_PATH, _T("%s%s"), _T(S3_IMG_UPDATE_DEST), _T(CONFIG_NAME));

	class S3Update U(argv[1], _T(S3_IMG_PAYLOAD_FILENAME),
		_T(S3_IMG_UPDATE_WRAP_FILENAME), ImgUpdateDest);
