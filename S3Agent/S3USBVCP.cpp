...
#include "S3GPIB.h"
#else
#define S3_CMD_TERMINATOR		0x03 // Line feed/0xA/10
#endif
...
CString CS3USBVCP::Read()
{
    DWORD dwBytes; //, dwCommEvent;
	CString	str;
    char buf[DEFAULT_BUFLEN], endchar = 0x00;
    ...
    while(endchar != 0x03)
    {
        ...
        {
            endchar = 0x03;
            str = "ERROR: Nothing on Serial Port";
        }
...
BOOL CS3USBVCP::OpenPort(CString PortName)
{
	...
	if (m_hPort == INVALID_HANDLE_VALUE)
	{
		...
		// Open the port
		m_hPort = CreateFile (
					_T("\\\\.\\") + PortName,							// Pointer to the name of the port
